...
python_object = ['yee ku', {'favorite': ('coding', None, 'game', 25)}]
print(type(python_object))
s = json.dumps(python_object)
print(type(s))  # ["yee ku", {"favorite": ["coding", null, "game", 25]}]
# 简单的Python字符串转JSON
...
json.dump(['Kotlin', {'Python': 'excellent'}, 'a'], f)


# 将JSON字符串恢复成Python列表
result1 = json.loads('["yee ku", {"favorite": ["coding", null, "game", 25]}]')
print(result1)  # ['yee ku', {'favorite': ['coding', None, 'game', 25]}]
...
result3 = json.loads('{"__complex__": true, "real": 1, "image": 2}', object_hook=as_complex)
print(result3)  # (1+2j)
f = open('a.json')
# 从文件流恢复JSON列表
result4 = json.load(f)
